# Nuevo
- Nueva interfaz de inicio
- Ahora se pueden descargar/guardar los mundos con un nuevo botón en celular, o Ctrl+S en PC
- Ya se pueden cargar mundos creados con la extensión .silky

# SilkyWorld
Una recreación de una versión sencilla de Minecraft hecho enteramente con Claude IA y añadidos, como simuladores de tela (se aceptan forks para mejorarlo)

## Cómo se juega
### PC:
- WASD = Mover el personaje
- Click izquierdo = Romper un bloque
- Click derecho = Colocar un bloque, Tela o remera en el suelo
- E = Agarrar un bloque con los dos brazos y llevarlo a otro lugar
- R = Agarrar las Telas o Remeras
- F = Cambiar la perspectiva de jugador
- G = Sentarse
- V = Activar o desactivar modo Fly (Flotante)
- Click derecho mientras sostienes una tela o remera = Equiparlo
- Espacio = Saltar
- Espacio mientras modo fly activado = Volar arriba
- G mientras modo fly activado = bajar un poco
- Números = cambiar de ítem (Parcialmente)
- Rueda del Mouse = Cambiar entre ítems de al lado
- Ctrl+S = Descargar mundo

### Celular: 
- El menú con todos los botones está en pantalla
- Joystick = Mueve el personaje
- Inventario está abajo
- Botón Paracaidas = activar modo Fly
- Botón Flecha arriba = Saltar
- Botón visto verde = Colocar bloques, Telas o remeras en el suelo
- Botón X = Quitar bloques
- Botón de Silla = Sentarse
- Botón ladrillo = agarrar un bloque con ambos brazos y llevarlo a otro lugar
- Botón mano o guante = Agarrar una Tela o Remera del suelo
- Botón Visto verde mientras sostienes una Remera = Equiparse la Remera
- Botón Visto verde mientras sostienes una Tela = Equiparse la Tela como Capa de personaje
- Botón Flecha arriba mientras modo fly activado = Volar hacia arriba
- Botón Silla mientras modo fly activado = bajar suavemente
- Botón Cohete = Desactivar modo Fly
- Botón ojo de arriba = Cambiar perspectiva
- Botón Guardar/Disquete = Descargar mundo (Guardar mundo)

## Bugs conocidos
- En celular el personaje no puede volar hacia arriba mientras tiene equipado una tela como capa (No sucede en PC, próximamente parcheando)
- La tela o el Toy Block puede irse arriba de un bloque que está como Techo (difícil de solucionar)
- Problemas de rendimiento en dispositivos gama baja o antiguos (Depende del hardware)
- El personaje atraviesa bloques colocados (Es necesario saltar para subirse arriba, esto no se puede arreglar de momento para que no suceda lo mismo de teletransporte hacia arriba como las telas debido a un bug)
- Otros bugs pequeños

## Items:
- Bloque de Césped (Pasto)
- Bloque de TIerra
- Bloque de Piedra
- Bloque de madera
- Bloque de ladrillo
- Bloque de Arena
- Bloque de Nieve
- Bloque de vidrio transparente
- Tela Roja
- Tela Azul
- Tela Amarilla
- Lanzador de Bloques de juguete empujables (ToyBlocks)
- Lanzador de Telas de colores aleatorios
- Remera color Blanco
- Remera color Rojo
- Remera color Azul
- Remera color Amarillo
- Próximamente más...

### Cómo instalarlo:
Simplemente descarga el código fuente y descomprímelo en una carpeta, abre el archivo .html y listo (No borres el otro archivo que viene con él ya que eso ocasionará que no vuelva a funcionar y tendrias que volver a descomprimirlo. Funciona en cualquier navegador moderno que soporte abrir archivos html (O puedes llevarlo a Electron sin problemas)

## Créditos
Este simulador de cubos es experimental, se intenta parecer un poco a Minecraft pero no plagiarlo directamente, sino solamente tomar su forma cúbica y convertirlo en algo para experimentar. Minecraft es una marca registrada y este juego no se relaciona directamente ni está afiliado directamente a él, Minecraft es propiedad de Mojang Studios, Microsoft Studios y Xbox games.

Este simulador está hecho con cariño por Matias01jr como parte de un experimento sobre programar con una IA (Claude) pidiendo que agregue y corrija de a poco cada cosa hasta lograrlo, y lograr un simulador que combina cubos y telas. Cualquier ayuda es bienvenida, sean forks, pull requests, etc. Se permite subir a páginas de juegos libremente o meterlo a Electron, ya que es un experimento sin fines de lucro, solo de entretenimiento y de experimento. Gracias por apoyar y por probarlo, espero que te guste :D
